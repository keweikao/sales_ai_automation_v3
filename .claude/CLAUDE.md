...
| `architecture-check` | 新增/修改 API 路由、Service 時 | 架構原則檢查、Service 層分離、DI 驗證 |
...
3. architecture-check  （確保架構原則）
4. tdd-guard  （確保測試覆蓋）
5. code-review  （程式碼品質審查）
6. security-audit  （深度安全檢查，視情況）
```

### 自動執行規則

1. **完成功能開發後**：自動執行 `code-review` + `typescript-quality`
2. **新增/修改 API 路由時**：自動執行 `architecture-check`
3. **準備 commit 前**：自動執行 `secret-scanner` + `tdd-guard`
...
- `/tdd-guard` - TDD 守護
- `/architecture-check` - 架構原則檢查（Service 層分離、DI、錯誤處理）
