...
describe("R2 儲存服務", () => {
	...
	describe("下載功能", () => {
		...
		test("應該處理檔案不存在的情況", async () => {
			...
			Object.assign(notFoundError, { name: "NoSuchKey", $metadata: { httpStatusCode: 404 } });
	...
	describe("連線失敗處理", () => {
		test("應該處理網路連線失敗", async () => {
			...
			const uploadFile = async (key: string, body: Buffer) => {
				return mockS3Client.send({ Bucket: "test-bucket", Key: key, Body: body });
			};

			await expect(
				uploadFile("test.mp3", Buffer.from("data")),
		...
		test("應該處理逾時錯誤", async () => {
			...
			await expect(
				mockS3Client.send({ Key: "test.mp3" }),
